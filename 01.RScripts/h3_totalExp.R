...
# Principales variables independientes, con controles e interacciones
# Efectos individuales nivel pais
h3_in3 <- plm(basic ~ VP_mean + KOFGI + v2edteunionindp_ord +
                (VP_mean * KOFGI * v2edteunionindp_ord) +
                pop_w1_ln + pop_w2 + eco_w4 + eco_w6 + eco_w16,
              data = h3,
              model = "within",
...
# Efectos de dos vias
h3_tw3 <- plm(basic ~ VP_mean + KOFFiGI + v2edteunionindp_mean +
                (Hrre_hogR * KOFFiGI * v2edteunionindp_mean) +
                pop_w1_ln + pop_w2 + eco_w4 + eco_w6 + eco_w16,
              data = h3,
              model = "within",
              effect = "twoways")
...
c_me %>%
  ...
  theme(plot.title = element_text(size = 10), 
        ...
        axis.text.y = element_text(color = "black", size = 8)) + 
  facet_wrap(~ v2edteunionindp_ord, 
             labeller = as_labeller(c('0' = "Sindicalismo Dependiente", 
                                      '1' = "Sindicalismo Moderado", 
                                      '2' = "Sindicalismo Independiente", 
                                      '3' = "Sindicalismo Autónomo")))
